...
//Run to see if supported

if(!supportLocalStorage()) {
 console.log("No HTML5 localStorageSupport");
}
    else {
       console.log("There is LocalStorage Support!") ;
    
        const notificationEmail= document.getElementById('email');
        localStorage.setItem('email', notificationEmail.checked);
        
        const notificationPublic= document.getElementById('public');
        localStorage.setItem('public', notificationPublic.checked);
        
        const valueTimeZone= document.getElementById('dropDownTimezone').value;
        localStorage.setItem('valueTimeZone', valueTimeZone);
    };
...
if(localStorage.lenght > 1) {
   ...
   const settingEmail = JSON.parse(localStorage.getItem('email'));
    document.getElementById("email").checked= settingEmail;
    
     const settingPublic= JSON.parse(localStorage.getItem('public'));
    document.getElementById("public").checked= settingPublic;
    
    const choiceTimeZone= localStorage.getItem('valueTimeZone');
    document.getElementById('dropDownTimezone').value= choiceTimeZone;
